...
export class RequestParser {
  ...
  static queryOptionsParser?: (
    ...
    parseResult: IResourceQueryOptions<any>,
    queryParams: IDict,
  ) => IResourceQueryOptions<any>;
  /**
  ...
  static async parseRequest(
    ...
    const inputData: IParseRequestConfigMap = {
      ...
      queryOptions:
        ...
        String(config).trim().startsWith('queryOptions')
          ? RequestParser.parseQueryOptions(ctx)
          : ({} as IResourceQueryOptions<any>),
    };
   ...
   */
  static parseQueryOptions<T extends IResourceData = IResourceData>(
    ctx: ExecutionContext,
  ): IResourceQueryOptions<T> {
    const req = ctx.switchToHttp().getRequest();
    const { queryParams, ...result } =
      ResourcePaginationHelper.parseQueryOptions(req);
    const r =
      typeof RequestParser.queryOptionsParser === 'function'
        ? RequestParser.queryOptionsParser(ctx, result, queryParams)
        : result;
 ...
 * ```
 */
export const ParseRequest = createParamDecorator<IParseRequestConfig>(
