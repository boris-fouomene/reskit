...
import { Validator, IValidatorValidateOptions, IValidatorRuleFunction } from "@resk/core/validator";
```
...
const passwordValidation = await Validator.validate({
  ...
  rules: ["required", "string", "minLength[8]", "maxLength[50]", "regex[/^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d).+$/]"],
  fieldName: "password",
...
const passwordValidation = await Validator.validate<FormContext>({
  ...
  rules: [
    ...
    "minLength[8]",
    ({ value, context }) => {
      return value !== context.currentPassword || "New password must be different from current password";
    },
  ],
...
**Returns:** `IValidatorRules` - Object containing all registered rules
...
```typescript
const mixedRules = ["required", "minLength[3]", { maxLength: [50] }, ({ value }) => value.includes("@") || "Must contain @"];
...
```typescript
type IValidatorRule<ParamType extends Array<any> = Array<any>, Context = unknown> = IValidatorRuleFunction<ParamType, Context> | IValidatorRuleName | `${IValidatorRuleName}[${string}]` | Record<IValidatorRuleName, ParamType>;
```
...
```typescript
type IValidatorRuleFunction<ParamType extends Array<any> = Array<any>, Context = unknown> = (options: IValidatorValidateOptions<ParamType, Context>) => IValidatorResult;
```
...
Validator.registerRule("fileSize", ({ value, ruleParams }) => {
  ...
  return value.size <= maxSizeInBytes || `File size must not exceed ${maxSizeInMB}MB`;
});
...
Validator.registerRule("ValidDomain", async ({ value }) => {
  try {
    ...
    return (data.Status === 0 && data.Answer?.length > 0) || "Domain does not exist or is not reachable";
  } catch (error) {
...
Validator.registerRule("PasswordStrength", async ({ value, ruleParams }) => {
  ...
  try {
    ...
    return score >= minimumScore || `Password strength must be at least ${minimumScore}/5`;
  } catch (error) {
...
const validateRegistrationForm = async (formData: RegistrationForm) => {
  const validationTasks = [
    ...
    Validator.validate({
      ...
      rules: ["Required", "String", "MinLength[8]", "Regex[/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]/]"],
      fieldName: "password",
      translatedPropertyName: "Password",
    }),
...
const validateProduct = async (product: Product) => {
  ...
  const errors = results.filter((result): result is PromiseRejectedResult => result.status === "rejected").map((result) => result.reason.message);
  ...
  if (profile.username !== context.currentUser.username) {
    validations.push(
      Validator.validate({
        ...
        rules: ["Required", "String", "MinLength[3]", "MaxLength[30]", "AlphaDash", "UniqueUsername"],
        fieldName: "username",
        context,
      })
    );
  ...
  if (context.isPasswordChange) {
    validations.push(
      ...
      Validator.validate({
        ...
        rules: ["Required", "String", "MinLength[8]", "Different[currentPassword]", "StrongPassword"],
        fieldName: "newPassword",
...
| `getRules`                  | None                                      | `IValidatorRules`                    | Get all registered rules                   |
