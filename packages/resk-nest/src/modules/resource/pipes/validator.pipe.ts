...
import { IClassConstructor, IDict } from '@resk/core/types';
 ...
 * ```
 */
export const ValidatorParam = createParamDecorator<
  | [config: IParseRequestConfig, dtoClassParam: IClassConstructor<any>]
  | IParseRequestConfig
>(async (options, ctx: ExecutionContext) => {
  /**
   ...
   */
  const { dtoClassOrGetDtoMethodName, optional } = Object.assign(
    {},
    Reflect.getMetadata(validatorPipeDtoMetadataKey, handler),
  );
  /***
   * The validator params become optional when the dtoClassOrGetDtoMethodName is undefined or the optional is true
   */
  let cannotValidate =
    optional === true || dtoClassOrGetDtoMethodName === undefined;
  let dtoClass: IClassConstructor | undefined =
    typeof dtoClassParam !== 'boolean' ? dtoClassParam : undefined;
  if (typeof dtoClassOrGetDtoMethodName === 'function') {
    ...
    if (!cannotValidate) {
       ...
       */
      if (!isNonNullString(dtoClassOrGetDtoMethodName))
        throw new BadRequestException(
          'Invalid Dto method name for validator : ' + errorDetails,
        );
       ...
       */
      if (
        typeof controller.prototype[dtoClassOrGetDtoMethodName] !== 'function'
      ) {
        throw new BadRequestException(
          'Invalid dtoClass method : The method ' +
            dtoClassOrGetDtoMethodName +
            ' is not a function in controller ' +
            controllerName +
            ' : ' +
            errorDetails,
        );
    ...
    if (
      isNonNullString(dtoClassOrGetDtoMethodName) &&
      typeof controller.prototype[dtoClassOrGetDtoMethodName] == 'function'
    ) {
      /**
  ...
  if (!hasDtoValid) {
    throw new BadRequestException(
      'Invalid return type for dtoClass getter ' +
        dtoClassOrGetDtoMethodName +
        ' is not a function in controller ' +
        controllerName +
        ' : ' +
  ...
  try {
    return await ValidatorPipe.staticTransform(
      { dtoClass, data },
      {} as any,
      ctx,
    );
    ...
    Logger.error(
      'Validation failed for controller ' +
        controllerName +
        ' and handler ' +
        handlerName,
      e,
 ...
 * }
 */
export function UseValidatorParam<T extends InstanceType<any> = any>(
  dtoClassOrGetDtoMethodName: IClassConstructor<any> | keyof T,
  optional?: boolean,
): MethodDecorator {
  /**
   * Returns a method decorator that enables validation for a controller method.
...
export class ValidatorPipe implements PipeTransform<IDict, Promise<IDict>> {
  ...
  async transform(value: IDict, metadata: ArgumentMetadata): Promise<IDict> {
    ...
    throw new BadRequestException(
      'Invalid dtoClass for validator pipe ' + this.constructor.name,
    );
  }
   ...
   * @throws {BadRequestException} If the validation fails.
   */
  static async staticTransform(
    options: IValidatorConfig,
    ...
    if (!options.dtoClass || typeof options.dtoClass !== 'function') {
      throw new BadRequestException(
        `Invalid value type ${typeof options.data} input for Validator pipe line, data : ` +
    ...
    if (Array.isArray(data)) {
      ...
      const promises: Promise<any>[] = data.map((v: any, index) => {
        return new Promise((resolve, reject) => {
          Validator.validateTarget<typeof dtoClass, ExecutionContext>(
            dtoClass,
            v,
            { context },
          )
            ...
            .catch((err: any) => {
              errors.push({
                message: isNonNullString(err?.message)
                  ? err.message
                  : stringify(err),
                data: v,
                index: index + 1,
      ...
      if (errors.length) {
        throw new BadRequestException({
          message: i18n.translate('validator.failedForNItems', {
            count: errors.length,
          }),
          errors,
        });
    ...
    try {
      await Validator.validateTarget<typeof dtoClass, ExecutionContext>(
        dtoClass,
        data,
        { context },
      );
