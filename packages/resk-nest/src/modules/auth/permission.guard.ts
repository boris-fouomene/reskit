...
export const Permissions = (...permissions: IAuthPerm[]) => {
  return buildPropertyDecorator<IAuthPerm[]>(PERMISSIONS_KEY, permissions);
};
...
export class PermissionsGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
     ...
     */
    const requiredPermissions = getDecoratedProperty<IAuthPerm[]>(
      context.getClass(),
      PERMISSIONS_KEY,
    ...
    if (!canActivate) {
      throw new ForbiddenException(
        i18n.translate('auth.guards.permissions.forbiddenError'),
      );
    }
    return true;
  }
