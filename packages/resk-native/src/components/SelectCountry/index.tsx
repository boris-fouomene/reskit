 ...
 */
export function SelectCountry({ withLabel, label, anchorProps, textColor, iconSize, ...props }: ISelectCountryProps) {
    ...
    return <Dropdown<ICountry, ICountryCode>
        ...
        getItemLabel={({ item }) => {
            return <View style={styles.countryFlagContainer}>
                ...
                {isNonNullString(item.dialCode) && <Label>{"(" + item.dialCode + ") "}</Label>}
            </View>;
        }}
