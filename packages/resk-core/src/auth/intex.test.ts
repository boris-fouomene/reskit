...
import { IAuthPerm, IAuthPerms, IAuthUser } from "./types";
...
describe("Auth", () => {
  ...
  describe("signOut", () => {
    it("should successfully sign out a user", async () => {
      const testUser: IAuthUser = { id: "signout123" };
      await Auth.signIn(testUser, false);
    ...
    it("should clear session data completely", async () => {
      const testUser: IAuthUser = { id: "cleanup123", token: "test-token" };
      await Auth.signIn(testUser, false);
  ...
  describe("isAllowed", () => {
    ...
    it("should handle function permissions", () => {
      const user: IAuthUser = { id: "func123", perms: { documents: ["read"] } };

      const permFn = (u: IAuthUser) => u.id === "func123";
      expect(Auth.isAllowed(permFn, user)).toBe(true);

      const falseFn = (u: IAuthUser) => u.id === "different";
