...
export const isValidImageSrc = (src:any) : boolean => {
    if(!isNotEmptyString(src)) return false;
    src = src.trim();
...
export function isDataURL(s:string): boolean {
    return isNotEmptyString(s) && !s.includes("data:image/x-icon") && !!s.match(isDataURLRegex);
}
...
export const isValidUrl = (uri: any) : boolean =>{
    return isNotEmptyString(uri) && /^(?:\w+:)?\/\/([^\s\.]+\.\S{2}|localhost[\:?\d]*)\S*$/.test(uri);
};