...
export function Expandable({
    ...
    const { expanded, toggleExpand, isControlled } = useExpandable({ expanded: controlledExpanded, onExpandChange });
    const id = defaultStr(customId);
    const displayIconAtRight = iconPosition === "right";
    testID = defaultStr(testID, "resk-expandable")
    const icon = <ExpandableIcon
        expanded={expanded}
        ...
        className={cn("resk-expandable-icon", computedVariant.icon(), iconVariant(expandIconVariant), iconClassName)}
    />
    // Handle expandable button press
    /* const handlePress = ; */

    return <Div {...props} testID={testID} className={cn(`resk-expandable`, computedVariant.base(), className)} id={id}>
        <Div asHtmlTag="span" testID={testID + "-header-container"} className={cn(`resk-expandable-header-container w-full transition-all duration-200`, computedVariant.headerContainer(), headerClassName)}>
            <Div asHtmlTag="span" testID={testID + "-header"} className={cn("flex flex-row justify-start items-center", computedVariant.header())}>
